    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav">
                <li <c:if test="${nav_tab=='index'}">class="active"></c:if>> <a href="index"><i class="glyphicon glyphicon-cloud"></i>&nbsp;主&nbsp;&nbsp;页</a></li>
                <li <c:if test="${nav_tab=='note'}">class="active"></c:if>><a href="note?actionName=viewNote"><i class="glyphicon glyphicon-pencil"></i>&nbsp;发表云记</a></li>
                <li <c:if test="${nav_tab=='noteType'}">class="active"></c:if>><a href="noteType?actionName=noteType"><i class="glyphicon glyphicon-list"></i>&nbsp;类别管理</a></li>
                <li <c:if test="${nav_tab=='userCenter'}">class="active"></c:if>><a href="user.do?actionName=userCenter"><i class="glyphicon glyphicon-user"></i>&nbsp;个人中心</a>
                <li <c:if test="${nav_tab=='dataReport'}">class="active"></c:if>><a href="report?actionName=dataReport"><i class="glyphicon glyphicon-signal"></i>&nbsp;数据报表</a></li>
            </ul>
            <form class="navbar-form navbar-right" role="search" action="index">
                <div class="form-group">
                    <input type="hidden" name="actionName" value="searchTitle">
                    <input type="text" name="title" class="form-control" value="${title}" placeholder="搜索云记">
                </div>
        ...
        <div class="col-md-3">
            <div class="data_list">
                <div class="data_list_title"><span class="glyphicon glyphicon-user"></span>&nbsp;个人中心&nbsp;&nbsp;&nbsp;&nbsp;
                    <a href="user.do?actionName=logout">退出</a></div>
                <div class="userimg">
                    <img src="user.do?actionName=userHead&imageName=${user.head}">
                </div>
                ...
                <div>
                    <ul class="nav nav-pills nav-stacked">
                        <c:forEach items="${dateInfo}" var="item">
                            <li><a href="index?actionName=searchDate&date=${item.groupName}">${item.groupName}
                ...
                <div>
                    <ul class="nav nav-pills nav-stacked">
                        <c:forEach items="${typeInfo}" var="item">
                            <li id="li_${item.typeId}"><a href="index?actionName=searchType&typeId=${item.typeId}">
