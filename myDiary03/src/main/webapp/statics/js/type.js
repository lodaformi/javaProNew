...
function deleteDom(typeId) {
	...
	var myTable = $("#myTable");
	//	2、得到表格对象的子元素tr的数量
	var trLen = $("#myTable tr").length;
	//	3、判断tr的数量是否等于2
	console.log("myTable tr length: " + trLen);
	if (trLen == 2) {
		// 如果tr的数量等于2，表示只有一条类型记录，删除整个表格对象，并设置提示内容
		$("#myTable").remove();
		$("#pDiv").html("<h2>未查询到日记类型</h2>");
	}else {
 ...
 */
function openUpdateModal(typeId) {
	// 1、清空文本框和隐藏域的值
	$("#msg").html("");
	$("#typeId").val("");
	// 2、修改模态框的标题
	$("#myModalLabel").html("修改类型")
	// 3、将当前要修改的tr记录的id和名称，设置到模态框的隐藏域和文本框中
	$("#typeId").val(typeId);
	$("#typename").val($("#tr_"+typeId).children().eq(1).text());
	// 4、打开模态框
	$("#myModal").modal("show");
}
...
$("#btn_submit").click(function () {
	...
	$.ajax({
		...
		data: {
			typeName: typeName,
			typeId: typeId,
			actionName: "addOrUpdateType"
		},
		success: function (result) {
			// 如果code=0，表示失败，提示信息
			if (result.statusCode == 0) {
				$("#msg").html(result.msg)
			} else if (result.statusCode == 1) {  // 如果code=1，表示更新成功，
				// 关闭模态框
				$("#myModal").modal("hide");
				// 执行Dom操作
				// 判断typeId是否为空
...
function updateDom(typeId, typeName) {
	//1、修改指定tr记录
	$("#tr_"+typeId).children().eq(1).html(typeName);
	//2、左侧类型分组导航栏的列表项
	// 给左侧类型名添加span标签，并设置id属性值，修改该span元素的文本值
...
function addDom(typeId, typeName) {
	// 1、添加tr记录
	var strTr = "<tr id=tr_"+ typeId + ">" + "<td>" + typeId + "</td>" + "<td>" + typeName + "</td>";
	strTr += "<td>" + "<button className=\"btn btn-primary\" type=\"button\" onClick=\"openUpdateModal(${item.typeId})\">修改</button>&nbsp;";
	strTr += "<button className=\"btn btn-danger del\" type=\"button\" onClick=\"deleteType(${item.typeId})\">删除</button>";
	strTr += "</td></tr>";
	// 判断表格是否存在
	var table = $("#myTable");
	if (table.length > 0) {
		...
		// d、如果表格不存在，则拼接表格及tr对象，将表格设置到div中
		var div = $("#pDiv");
		var strTable = "<table id=\"myTable\" className=\"table table-hover table-striped \">" + "<tbody><tr>" +
			"<th>编号</th><th>类型</th><th>操作</th>" + "</tr>";
		strTable += strTr;
